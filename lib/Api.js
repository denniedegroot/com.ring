...
class Api extends Homey.SimpleClass {
    ...
    constructor() {
        ...
        setInterval(this._informAuthentication.bind(this), informAuthenticationInterval);
    ...
    _https (method, host, path, postdata, raw, callback) {
        // console.log('_https', method, host, path);
            ...
            response.on('end', () => {
                ...
                if (response.statusCode == 404) { //Not found
                    ...
                    try {
                        ...
                        // Homey.app.writeToTimeline("Data error " + raw + ' ' + response.statusCode + '-' + error + '-')
    ...
    _onSetSettings (name) {
        console.log('_onSetSettings', name, Homey.ManagerSettings.get(name));
        ...
        if (name === 'ringAccesstoken') {
            ...
            if (token == null) {
                this._authenticated = false;
                Homey.ManagerApi.realtime('com.ring.status', { state: 'unauthenticated'});
                return;
            }
        ...
        if (name === 'authenticationStatus') {
            let value = Homey.ManagerSettings.get(name);
    ...
    _informAuthentication () {
        console.log('_informAuthentication');

        if (!this._authenticated) {
            Homey.app.writeToTimeline('Homey is not authenticated at Ring please reauthenticate. ') 
        }
    }

    _setAuthenticationStatus (status, result) {
        // console.log('_setAuthenticationStatus', status, result);

        if (status) {
            if ( Homey.ManagerSettings.get('authenticationStatus') != 'Authenticated' ) {
                Homey.ManagerSettings.set('authenticationStatus', 'Authenticated');
                Homey.ManagerSettings.set('authenticationError', ''); 
            }
        } else {
            if (Homey.ManagerSettings.get('authenticationStatus') != result.error) {
                Homey.ManagerSettings.set('authenticationStatus', result.error);
                Homey.ManagerSettings.set('authenticationError', result.error_description);
            }
        }
    }
    ...
    userLocations() {
        // console.log(`userLocations: Send request to retrieve all user locations in Ring`);
